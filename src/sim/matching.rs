...
/// Match a LIMIT order against the book. Sweeps the opposite side
/// until either the order fills, the next level doesn't cross, or
/// the opposite side empties; any leftover is inserted on the
/// matching side at the incoming limit price.
pub fn match_limit(book: &mut OrderBook, mut taker: IncomingLimit) -> LimitMatchOutcome {
    ...
    let rested = if taker.quantity > Decimal::ZERO {
        let resting = Resting {
            id: taker.id,
...
mod tests {
    ...
    fn no_cross_just_rests() {
        ...
        let out = match_limit(&mut b, lim(1, Side::Buy, dec!(85.0), dec!(1.0)));
    ...
    fn full_match_against_single_resting() {
        let mut b = OrderBook::new();
        match_limit(&mut b, lim(1, Side::Sell, dec!(85.0), dec!(1.0)));
        let out = match_limit(&mut b, lim(2, Side::Buy, dec!(85.0), dec!(1.0)));
    ...
    fn taker_larger_than_resting_partial_rest() {
        let mut b = OrderBook::new();
        match_limit(&mut b, lim(1, Side::Sell, dec!(85.0), dec!(0.5)));
        let out = match_limit(&mut b, lim(2, Side::Buy, dec!(86.0), dec!(2.0)));
    ...
    fn sweeps_multiple_levels_in_price_order() {
        let mut b = OrderBook::new();
        match_limit(&mut b, lim(1, Side::Sell, dec!(85.0), dec!(0.5)));
        match_limit(&mut b, lim(2, Side::Sell, dec!(85.5), dec!(0.5)));
        match_limit(&mut b, lim(3, Side::Sell, dec!(86.0), dec!(0.5)));

        let out = match_limit(&mut b, lim(4, Side::Buy, dec!(86.0), dec!(1.2)));
    ...
    fn time_priority_at_same_level() {
        ...
        match_limit(&mut b, lim(1, Side::Sell, dec!(85.0), dec!(1.0)));
        match_limit(&mut b, lim(2, Side::Sell, dec!(85.0), dec!(1.0)));
        match_limit(&mut b, lim(3, Side::Sell, dec!(85.0), dec!(1.0)));

        // Buy 2.5 sweeps id 1 in full, id 2 in full, id 3 partially.
        let out = match_limit(&mut b, lim(4, Side::Buy, dec!(85.0), dec!(2.5)));
    ...
    fn taker_stops_at_non_crossing_level() {
        let mut b = OrderBook::new();
        match_limit(&mut b, lim(1, Side::Sell, dec!(85.0), dec!(0.3)));
        match_limit(&mut b, lim(2, Side::Sell, dec!(86.0), dec!(1.0))); // too expensive

        // Buyer limit at 85.0: only the 85.0 level is takeable.
        let out = match_limit(&mut b, lim(3, Side::Buy, dec!(85.0), dec!(2.0)));
        assert_eq!(out.fills.len(), 1);
    ...
    fn match_price_is_maker_price_not_taker() {
        ...
        let out = match_limit(&mut b, lim(2, Side::Buy, dec!(90.0), dec!(1.0)));
        assert_eq!(out.fills[0].price, dec!(85.0));
    }
}
